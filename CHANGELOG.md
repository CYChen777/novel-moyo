...
- 修复退出时 `App.OnExit` 使用启动时旧设置对象保存，导致用户刚保存的字体、透明度、热键等设置被旧值覆盖的问题。
- 修复启用“启动最小化到托盘”后主窗口未创建句柄，导致全局热键和鼠标钩子可能不注册的问题。
- 修复章节像素偏移计算未计入章节分隔符高度的问题；章节缓冲区追加、前置、裁剪后统一重测当前缓冲区的章节起点，减少长篇阅读时跳章、书签偏移和恢复进度偏移。
- 修复未完成下载任务在重启后被丢弃的问题；`Pending` / `Downloading` 任务会按原书源自动重新排队，书源不存在时标记失败并保留错误信息。
- 优化在线书城搜索体验：搜索结果先展示，章节数后台并发补齐，避免慢书源阻塞列表显示。
- 异步命令增加兜底异常处理，避免遗漏异常直接进入全局异常处理器。
- 热键解析或注册失败时显示 toast 提示，不再静默失败。
- 安装脚本改为相对路径，避免发布构建依赖本机 `D:\projects\novel-moyo`。

### Documentation

- 将“断点续传”描述调整为“任务恢复 / 重启后未完成任务自动重新排队”，与当前实现保持一致。
- 版本号更新到 `1.0.4`。
